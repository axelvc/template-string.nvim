...
function M.on_type()
	local node = U.get_string_node({ "string_literal", "raw_string_literal", "interpolated_string_expression" })
	if not node then
		return
	end
	...
	if M.is_string_literal(text) and M.has_interpolation(text) then
		-- "{foo}" -> $"{foo}"
		local new_text = "$" .. text
	...
	elseif not M.is_string_literal(text) and not M.has_interpolation(text) and C.options.remove_template_string then
		-- $"foo" -> "foo"
